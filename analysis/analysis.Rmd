...
hurricane_plants_long <- hurricane_plants %>%
  ...
  filter(value == "TRUE") %>%
  group_by(species, life_form, plant_type, phenophase) %>%
...
```{r main-lolipop-plot, warning=FALSE}
# plot of species phenophases facet by phase
hurricane_plants_long %>%
   # filter(plant_type == "woody") %>%
   # filter(plant_type == "herbaceous") %>%
   # filter(plant_type == "fern") %>%
   # filter(plant_type == "coniferous") %>%
ggplot() +
 ...
 ggsave("herbaceous-phenophases.png", width = 10, height = 6)
```

```{r calculate-special-phenophases, warning=FALSE}
...
```{r phenophase-lolipop, warning=FALSE}
...
```{r daily-mean-temp, fig.alt = "A line plot of the average daily temperatures measured from NERACOOS buoy F01. The lowest the temperature reached was nearly -10 degrees celcius in March and the highest temperatures were almost 20 degrees celcius in July, August, and September."}
